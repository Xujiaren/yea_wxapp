...
class mood extends Component<{}, PageState> {
	config = {
	...
	myMood = () => {
		Taro.navigateTo({
			url: './moodPage/myMood' + '?userId=' + this.state.user.userId
		})
	}
	onOther = (val) => {
		Taro.navigateTo({
		...
		return (
			<View className='box row col jc_ct'>
				...
				<View className='avatar'>
					<Image src={user.avatar} className='avatar_pic' onClick={this.myMood} />
				</View>
								...
								return (
									<View className='row mt_30'>
										<View className='avatar_itm'>
											<Image src={item.avatar} className='pic_i' onClick={this.onOther.bind(this, item)} />
										</View>
										<View className='row col ml_10'>
											<View className='label_three label_14 font_bold' onClick={this.onOther.bind(this, item)}>{item.nickname}</View>
													...
													<View>
														{
															item.galleryList[0].ftype == 0 ?
																<View className='pic_box row wrap mt_10'>
																	{
																		item.galleryList.map(ele => {
																			return (
																				<View className='pic_item mr_6 mt_5'>
																...
																: item.galleryList[0].ftype == 1 ?
																	<View className={item.moodId == idx ? 'videos bg_yellow mt_10' : 'video bg_yellow mt_10'} onClick={this.onPlay.bind(this, item)}>
																		<Video
																			id='video'
												...
												<View className='row ai_ct'>
													{
														item.isLike == false ?
															<View className='row' onClick={this.onVant.bind(this, item)}>
																...
																<Image src={asset.praise} className='size_24 mt_2' />
															...
															<View className='row' onClick={this.downVant.bind(this, item)}>
																...
																<Image src={asset.onpraise} className='size_24 mt_2' />
...
export default mood as ComponentClass